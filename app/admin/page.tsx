...
export default async function AdminPage() {
  ...
  const [restaurants, listings, users, contestVotes, heldVotes, reports, suggestions] = await Promise.all([
    supabase.from("restaurants").select("*", { count: "exact", head: true }),
    supabase.from("marketplace_listings").select("*", { count: "exact", head: true }),
    supabase.from("profiles").select("*", { count: "exact", head: true }),
    supabase.from("restaurant_votes").select("*", { count: "exact", head: true }),
  ...
  const stats = [
    ...
    ["Users", users.count ?? 0],
    ["Contest votes", contestVotes.count ?? 0],
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-7">
        {stats.map(([label, value]) => (
          <div key={label} className="card p-5">
            <p className="text-sm font-bold text-[#687068]">{label}</p>
