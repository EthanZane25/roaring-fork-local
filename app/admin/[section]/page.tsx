        ...
        <div className="card mt-8 overflow-hidden">
          {(listings ?? []).map((item) => (
            <div key={item.id} className="grid gap-4 border-b border-[#e2e4de] p-4 last:border-0 lg:grid-cols-[1fr_240px] lg:items-center">
              ...
              <form action={updateStatus} className="flex gap-2"><input type="hidden" name="table" value="marketplace_listings" /><input type="hidden" name="id" value={item.id} /><select name="status" defaultValue={item.status} className={`${FIELD} min-w-0 flex-1`}><option value="draft">Draft</option><option value="held">Held for review</option><option value="active">Active</option><option value="sold">Sold</option><option value="expired">Expired</option><option value="removed">Removed</option></select><button className={SAVE}>Save</button></form>
            </div>
          ))}
