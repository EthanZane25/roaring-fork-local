...
export async function POST(request: NextRequest) {
  ...
  const turnstile = devBypass
    ? { ok: true, skipped: true }
    : await verifyTurnstile(token, "create_listing", ip);

  if (
    !devBypass &&
    (!turnstile.ok ||
      (process.env.NODE_ENV === "production" && turnstile.skipped))
  ) {
  ...
  const admin = devBypass ? null : createAdminClient();
  ...
  if (!devBypass) {
    ...
    if (identityError) {
      return NextResponse.json(
        { error: "Unable to verify your account security status." },
        { status: 503 }
      );
    }
    ...
    if (identity?.banned_at) {
      return NextResponse.json(
        { error: "This account cannot post Marketplace listings." },
        { status: 403 }
      );
    }
    ...
    if (!identity?.email_verified_at) {
      return NextResponse.json(
        { error: "Verify your email before posting a listing." },
        { status: 403 }
      );
    }
  ...
  if (!devBypass) {
    try {
      ...
      return NextResponse.json({
        error: "Marketplace security controls are temporarily unavailable."
      }, { status: 503 });
    }
  }

  let firstListing = false;
  ...
  if (!devBypass) {
    ...
    if (countError) {
      return NextResponse.json(
        { error: "Unable to verify listing history." },
        { status: 503 }
      );
    }

    firstListing = (priorListingCount ?? 0) === 0;
  }

  if (firstListing && !devBypass) {
  ...
  const listingStatus =
    devBypass ? "active" : firstListing ? "held" : "active";
