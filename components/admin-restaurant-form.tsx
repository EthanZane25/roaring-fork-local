...
import { CUISINES, TOWNS } from "@/lib/constants";
  ...
  return (
    <form onSubmit={submit} className="card p-7">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label className="grid gap-2"><span className="text-sm font-bold">Primary cuisine</span><select name="cuisine" required defaultValue={initial?.primary_cuisine || "other"} className="rounded-md border border-[#d6d8d1] px-4 py-3">{CUISINES.filter((item) => item.value).map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
        ...
        <label className="grid gap-2 sm:col-span-2"><span className="text-sm font-bold">Image URL</span><input name="imageUrl" type="url" defaultValue={initial?.image_url} className="rounded-md border border-[#d6d8d1] px-4 py-3" /><span className="text-xs font-normal text-[#687069]">Restaurant photos appear on the restaurant detail page; paid placements can still be labeled Sponsored.</span></label>
